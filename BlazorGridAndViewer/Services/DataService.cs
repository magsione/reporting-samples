...
namespace Blazor_UI_and_Report_Viewer.Services
{
    public class DataService
    {
        ...
        public DataService(AdventureContext dbContext)
        {
            _dbContext = dbContext;
        }

        public async Task<List<ProductCategoryBindingModel>> GetProductSubCategoriesAsync()
        {
            return (await _dbContext.ProductSubCategory.Include(sc => sc.ParentProductCategory).ToListAsync())
                .Select(ProductCategoryBindingModel.ProductCategoryFunc).ToList();
        }
        ...
        public async Task<List<ProductBindingModel>> GetProductsAsync(int? categoryId = null, bool filterDefaultImages = true)
        {
            if (!productData.Any())
            {
                productData = (await _dbContext.Product.Include(p => p.ProductSubcategory).ToListAsync())
